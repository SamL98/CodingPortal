...
func getArticles(db *sql.DB, table string) []article {
	...
	rows, err := db.Query(fmt.Sprintf("select id, article_id, text, user_id from %s", table))
	if err != nil {
...
func getAnswers(db *sql.DB) [][]string {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&row[0], &row[1], &row[2], &row[3], &row[4], &row[5], &row[6], &row[7], &row[8], &row[9], &row[10], &row[11], &row[12], &row[13], &row[14], &row[15], &row[16], &row[17], &row[18]); err != nil {
